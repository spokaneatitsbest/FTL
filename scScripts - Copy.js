...
window.onload = function() {
    //load canvas
    function audioCreate(path,volume) {
...
function hyperJump(pnum, exec) { //HYPERJUMP!
    if (pnum == 1) {if (fuel1 > 0) {new Audio('audio/teleport/teleport.ogg').play().volume(.3); p1x = Math.floor((Math.random() * 6)*200); p1y = Math.floor((Math.random() * 6)*100); fuel1 = fuel1-1; console.log(fuel1); $("#f1").text(fuel1);} else {if (exec == 0){$("#f1").text("0");} else {
